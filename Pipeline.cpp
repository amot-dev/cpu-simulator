...
void Pipeline::fetch(){
	...
	if (!instructionBits[31] && !instructionBits[30]){						//for the R-Type
		...
		IQ.load(opCode.to_ulong(), dest.to_ulong(), RegFile.getRegValidity(src1.to_ulong()), src1.to_ulong(), RegFile.getRegValidity(src2.to_ulong()), src2.to_ulong());
	}
		...
		IQ.load(opCode.to_ulong(), dest.to_ulong(), RegFile.getRegValidity(src1.to_ulong()), src1.to_ulong(), true, -1);
	}
